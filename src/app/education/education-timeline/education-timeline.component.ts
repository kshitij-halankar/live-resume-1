...
@Component({
  ...
  styleUrls: [ "./education-timeline.component.scss" , "education-timeline.component.reponsivity.scss"]
})
export class EducationTimelineComponent implements OnInit, OnDestroy {
  ...
  @Input() get currentField(): number {
    return this._currentField;
  }

  set currentField(value: number) {
    if(value) {
  ...
  @Input() get education(): IEducation[] {
      return this._education;
  }

  set education(value: IEducation[]) {
      if(value) {
  ...
  updateTimelineNavigation() {
    ...
    const targetElem = this.line.nativeElement.querySelector(`div[id-position="${this.currentField}"]`);
  ...
  populateEducationTimeline(): void {

    let dates: string[] = this._education.map(education => education.startAt);
